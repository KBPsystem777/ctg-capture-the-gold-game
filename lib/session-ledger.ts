...
const BUYERS = ['Central Bank of Philippines', 'Reserve Bank', 'Global Metal Fund']
...
export function generateSessionLedger(count = 8): { id: string; transactions: LedgerTransaction[]; hash: string } {
  const sessionId = crypto.randomBytes(16).toString('hex')
  const transactions: LedgerTransaction[] = Array.from({ length: count }).map((_, idx) => {
