...
- ✅ Think First, Code Second: Before writing any code, always scan the codebase for existing logic
  or components that can be reused or refactored for high reusability.
- ✅ Build for Reusability & Whitelabelling: Always design features and components with a
  whitelabelling mindset, ensuring they are generic, modular, and easily adaptable.
...
- ✅ Use Modern React (hooks, hoisted functional components)
- ✅ Use the existing design system in the codebase (ShadCN, Radix, etc.) while ensuring
  consistency, accessibility, and responsiveness and following best and modern practices.
- ✅ Always implement good ui/ux practices.
...
Unit-Test Friendly: Always ensure 90%+ test coverage and provide suggested Jest/React Testing
Library tests.

Documented: Use JSDoc comments to make the code engaging while keeping the documentation concise,
...
Minimal Re-Renders: Suggest best state management practices, as much as possible stay away from
external libraries. Let's prioritize React's native state management capabilities.

Unit Tests: Provide at least three Jest/RTL test cases covering all possible states. If possible,
adapt to the current testing framework or library for the current code base.

Clear Documentation: Add JSDoc + short inline comments

Create components in an arrow function way (Hoisted). Example:
...
Avoid bloated explanations—keep it short.
...
First, use React’s built-in state management before considering external libraries.

Only use external state management which is Zustand when absolutely necessary, such as for global
state that must persist across components.

when using Zustand, help me write a very simple code and workflow, ensure that the store is modular
and can be easily extended or replaced.

Ensure that state is managed in a way that minimizes re-renders and maximizes performance.
